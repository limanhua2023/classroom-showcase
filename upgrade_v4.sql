...
alter table activities add column if not exists feedback_daily_limit integer default 5;
alter table activities add column if not exists archive_after_days integer default 30;
alter table activities add column if not exists archive_delete_primary_after_success boolean default false;
alter table activities add column if not exists quarantine_retention_days integer default 3;
...
alter table student_roster add column if not exists feedback_muted boolean default false;
...
update student_roster set feedback_muted = false where feedback_muted is null;
update activities set feedback_daily_limit = 5 where feedback_daily_limit is null;
update activities set archive_after_days = 30 where archive_after_days is null;
update activities set archive_delete_primary_after_success = false where archive_delete_primary_after_success is null;
update activities set quarantine_retention_days = 3 where quarantine_retention_days is null;
...
alter table submissions add column if not exists quarantined_at timestamptz;
...
where status = 'quarantined_missing_media'
